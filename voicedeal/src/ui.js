// HTML-pagina's voor VoiceDeal. Eén loginpagina en één app-pagina.
// De app gebruikt de Web Speech API van de browser voor spraak-naar-tekst
// (werkt het best in Chrome op desktop en Android).
...
const STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  .card {
    ...
    box-shadow: 0 10px 40px rgba(0,0,0,.08); padding: 1.75rem;
...
export function appPage({ connected, redirectUri }) {
  const connectBanner = connected
    ? ""
    : `<div class="banner">
         <strong>Nog niet verbonden met Teamleader.</strong><br/>
         Registreer in je Teamleader-integratie deze redirect-URL:<br/>
         <code>${redirectUri}</code>
         <div class="spacer"></div>
         <a class="primary" href="/oauth/start" style="display:inline-block;text-decoration:none;padding:.6rem 1rem;border-radius:10px;">Verbind met Teamleader</a>
       </div>`;
  ...
  return page(
    ...
    `<div class="card">
      <div class="topbar">
        <h1>VoiceDeal</h1>
        <form method="POST" action="/logout"><button class="ghost" type="submit">Uitloggen</button></form>
      </div>
      ${connectBanner}

      <div class="center">
        <button id="mic" class="primary mic" type="button" title="Start/stop opname">🎙️</button>
      ...
      <div class="spacer"></div>
      <div class="row">
        <button id="send" class="primary" type="button" ${connected ? "" : "disabled"}>Verstuur naar Teamleader</button>
    ...
    <script>
      ...
      const connected = ${connected ? "true" : "false"};
      ...
      if (!SR) {
        ...
        recognition.onerror = (e) => {
          statusEl.textContent = "Fout bij spraakherkenning: " + e.error;
          stopRecording();
        };
        recognition.onend = () => {
